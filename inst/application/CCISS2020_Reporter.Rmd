...
pointDat <- data.table::fread("./test_locations/WilliamsLake_Locations.csv")
...
CCISS_Summary <- dplyr::left_join(CCISS_Summary,ID1_testsite) %>% dplyr::select(ID1, everything())#%>% filter(!ID1 == "test")
CCISS_Sites <- CCISS_Summary %>% dplyr::select(ID1, Lat, Long)

CCISS_Raw <- as.data.frame (cciss_res$Raw)
CCISS_Raw  <- dplyr::left_join(CCISS_Raw ,ID1_testsite) %>% dplyr::select(-ID2, -ModAgree, - SuitDiff, -Lat, -Long, -Elev) %>% 
  dplyr::select(ID1, dplyr::everything()) %>% 
  dplyr::filter(!ID1 == "test") %>% dplyr::mutate(dplyr::across(NewSuit, round, 0)) %>% dplyr::mutate(dplyr::across(c(`1`, `2`, `3`, X), round, 2))
