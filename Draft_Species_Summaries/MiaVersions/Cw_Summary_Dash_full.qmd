...
# each user must set this directory for your own location of the ccissr R project:
dir <- "C:/Users/miawater/Documents/ccissr/" # local copy

#dem2 <- rast(paste0(dir, "dem_BC2kmGrid.tif"))
...
#Set up database
con <- dbCon_cciss(paste0(dir, "bc_2km.duckdb"), threads = 8)
...
# Set up for bubble plots 
  # Calculate species persistence and expansion

DBI::dbExecute(con, "PRAGMA max_temp_directory_size='20GiB'") #if needed

for (sp in c(
  "Pl", "Fd", "Cw", "Sx", "At", "Py", "Ba", "Bl", "Bg",
  "Yc", "Hm", "Lw", "Hw", "Dr", "Ep", "Act", "Sb", "Mb", "Ss"
)) {
  perexp <- spp_persist_expand(con, spp_list = sp, fractional = TRUE)
} #run for one species at a time if machine can't handle more, then run this to cache them all together:
...
spp_bubbleplot(persist_expand, period = "2041_2060", scenario = "ssp245", species.focal = "Cw", edatope = c("C4")) 
```
:::
...
spp_bubbleplot(persist_expand, period = "2041_2060", scenario = "ssp245", species.focal = "Cw", edatope = c("B2")) 
```
:::
...
spp_bubbleplot(persist_expand, period = "2041_2060", scenario = "ssp245", species.focal = "Cw", edatope = c("D6"))
```
:::
:::
...
```{r}
##| title: Change in environmental suitability
##| fig-cap: 
#| out-width: 70%
