...
bgcNum = 1;
bgcFlags <- data.table(bgc = bgc_opts, removeFlag = NA_)
for(bgc_select in bgc_opts){
  ...
  if(length(pNums) > 25){
    ...
    if(nrow(feasSum[Curr %in% c(1,2) & !ccissFeas %in% c(1,2,3),] < 0)){
      bgcFlags[bgc == bgc_select, flag := ]
    }
    
    siteseries <- unique(feasSum$SS_NoSpace)
    cat("\n")
    cat("## Site_Unit: ",bgc_select," {#section",bgcNum,"}\n",sep = "")
    # print(
    #   tagList(tags$hr(style = "padding: 0; margin: 0 0 3px 0; height: 2px; background-color: purple; border: 0px"),
    #           tags$h2(paste0("Site_Unit: ",bgc_select," {#",bgc_select,"}")),
